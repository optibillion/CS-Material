...
export default function Batches() {
  ...
  async function fetchAll() {
    ...
    setBatches(b || []); setLoading(false)
  }

  async function handleSave(name) {
    const { error } = await supabase.from('batches').insert({ name, is_active: true })
        ...
        <table className="w-full min-w-[600px]">
          <thead>
            <tr className="border-b border-[#2a2a45]">
              {['BATCH NAME', 'STATUS', 'ACTIONS'].map(h => (
          ...
          <tbody className="divide-y divide-[#2a2a45]">
            {loading ? [...Array(4)].map((_, i) => (
              ...
              <tr><td colSpan={3} className="text-center text-[#6b7280] py-10 text-sm">No batches found</td></tr>
            ) : filtered.map(b => (
